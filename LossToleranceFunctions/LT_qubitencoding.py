...
def MC_decoding(poss_stabs_list, transmission, in_qubit=0, printing=False, provide_measures=False):
    ...
    if on_track:
        if provide_measures:
            return True, (tuple(measured_qubits), tuple(lost_qubits))
        else:
            return True
    else:
        if provide_measures:
            return False, (tuple(measured_qubits), tuple(lost_qubits))
        else:
            return False
