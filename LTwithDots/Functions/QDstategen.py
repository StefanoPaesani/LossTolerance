...
if __name__ == '__main__':
    ...
    from LTwithDots.Functions.LTqubitencoding import get_possible_stabs_meas, trasmission_scan_MCestimate, \
        find_best_graphs
    from CodesFunctions.local_complementation import lc_equivalence_class, lc_equivalence_class_full, \
        check_isomorphism_with_fixednode
    ...
    #
    #         if include_lc:
    #             if not check_isomorphism_with_fixednode(graph, obt_graphs, fixed_node=input_qubit):
    ...
    # print('All obtained graphs:')
    # num_graphs = len(obt_graphs)
    # num_factors = factorization(num_graphs)
    # n = num_factors[np.argmin(np.abs(np.array(num_factors) - np.sqrt(num_graphs)))]
    #
    # n_plot_rows = n
